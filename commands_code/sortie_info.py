...
class Sortie(commands.Cog):
    ...
    async def sortie(self, ctx):
        ...
        dat = get_all_sorties()
        if dat:
            await ctx.send(f"{flatten_list(dat[SORTIE_MISSIONS_KEY], True, True) + dat[TIME_KEY]}\n{ctx.author.mention}")
...
def get_all_sorties():
    ...
    for mission in response[SORTIE_MISSIONS_KEY]:
        if mission[MISSION_TYPE_KEY] == "Assassination":
            out[SORTIE_MISSIONS_KEY].append(f"{mission[MISSION_TYPE_KEY]}({response[BOSS_KEY]}) - {mission[MISSION_MOD_KEY]}")
